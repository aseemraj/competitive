...
node* build_persistent_ST(node *par, int B, int E, int num)
{
    if(B <= num && num < E)
    {
        ...
        {
            trav->LEFT = par->LEFT;
            trav->RIGHT = build_persistent_ST(par->RIGHT,mid,E,num);
        }
...
void dfs(int curr, int prev)
{
    ...
    if(prev == -1)
        root[curr] = build_persistent_ST( null, 0, maxi, v[curr].S);
    else
        root[curr] = build_persistent_ST( root[prev], 0, maxi, v[curr].S);
...
int query(node *a, node *b, node *c, node *d, int l, int r, int k)
{
    ...
    return query(a->RIGHT, b->RIGHT, c->RIGHT, d->RIGHT, mid, r, k-cnt);
}
